...
const fetchData = async () => {
  const response = await AxiosInstance.get(`/grade_eight/`);
...
const Grade8Table = () => {
  ...
  const { data: myData = [], isLoading, error, isFetching } = useQuery('grade_eightData', fetchData);
  ...
  const handleDelete = async (row) => {
    try {
      await AxiosInstance.delete(`/grade_eight/${row.original.id}/`);
      queryClient.invalidateQueries('grade_eightData');
  ...
  useEffect(() => {
    console.log('Fetching data for Grade Eight...');
  }, [myData]);
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: "self_image", header: "Self Image", size: 200 },
      { accessorKey: "worry", header: "Worry", size: 200 },
      { accessorKey: "neg_perl_rel", header: "Neg.Perl.Rel", size: 200 },
      { accessorKey: "antisocial", header: "AntiSocial", size: 200 },
      { accessorKey: "lie", header: "Lie", size: 200 },
      { accessorKey: "problem_index", header: "Problem Index", size: 200 },
      ...
      { accessorKey: "a_s", header: "AS", size: 200 },
    ...
    >
      ...
      <MaterialReactTable 
          ...
          <Dialog open={open} onClose={handleClose} fullWidth maxWidth="md">
              <DialogTitle>Edit Grade Eight Form</DialogTitle>
              <DialogContent>
                <Grade8 initialData={editData} onClose={handleClose}/>
              </DialogContent>
            </Dialog>
